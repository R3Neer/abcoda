...
import {
  CanonicalDraftTransformer,
} from "../../apps/widget/src/adapters/local/canonical-draft-transformer";
...
describe("CanonicalDraftTransformer", () => {
  ...
  it("transposes one voice without changing the global key or percussion", () => {
    const transformer =
      new CanonicalDraftTransformer();

    const result =
      transformer.transposeVoice(
        mixed,
        "P",
        2,
      );
  ...
  it("rejects tonal transposition for percussion voices", () => {
    const transformer =
      new CanonicalDraftTransformer();
