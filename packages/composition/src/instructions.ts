...
Follow routed effort/difficulty and instrument guidance. Use slurs, ties, articulation, dynamics, hairpins, breaths, fermatas, ornaments and techniques only when functional. For suitable piano styles from standard/intermediate upward, mark clear Ped./release points; avoid automatic historical/dry pedal and never claim synth pedalling.

For render_score, send complete ABC without fences. Keep headers, voice/group IDs, durations, meter-aware beams, clefs/transposition/ranges, repeats, tempo, instruments and voiceKinds consistent. Use ordinary clef changes for sustained ledger-line problems; reserve octave clefs (\`treble-8\`, \`treble+8\`, etc.) for genuine octave-transposing notation. abcjs sounds that octave, so do not duplicate it in \`transpose=\`. Put an inline clef before its first note. ABC spaces split beams. Unpitched voices need percussion plus intentional GM pitches. Playback omits pedal and some engraved techniques.
`.trim();
